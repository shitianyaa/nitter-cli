...
| `chooser: upstream_unavailable: no instances configured` | 1 | A Nitter-path fetch ran with an empty instance set — `list`/`watch`, a `--instance`-pinned fetch, or a mix-mode fallback after FxTwitter failed (that variant then appends the fast lane's cause in parentheses: `(fx attempt: ...)`) | Same as above: configure an instance first. Note that mix mode (default) serves `user`/`search`/`get`/`comments`/`following`/`profile`/`quotes`/`trends` without any instance |
| `fxtwitter.SearchTweets` / `fxtwitter.SearchUsers` / `fxtwitter.FetchQuotes`: `not_found: resource not found (404)` | 1 | The FxTwitter route failed. `/2/search`, `/2/search/users` and `/2/status/{id}/quotes` share one upstream query and answer 404 intermittently (window-dependent: ≈85% failures in the 2026-09-24 sample window, and zero-failure windows the same day). The CLI reports this instead of printing an empty result | Retry later — the endpoint recovers on its own. For a specific account use `profile <HANDLE>` rather than `search --type user`. A genuinely empty result is unaffected: it exits 0 with an empty list, and `quotes` on a tweet with zero quotes still exits 0 |
...
| `config set <key>: "..." is not an integer / not a duration / is invalid` | 2 | Value failed schema validation | Follow the documented shapes: `default_limit`/`max_pages` are integers >= 1 (caps — `0` is not "unlimited"), `retry_attempts` is >= 0, `request_interval`/`retry_delay`/`instance_cooldown` are durations >= 0 (`500ms`, `2s`), `log_level` is debug\|info, `log_format` is text\|json |
