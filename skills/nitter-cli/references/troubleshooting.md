...
| `chooser: upstream_unavailable: no instances configured` | 1 | A Nitter-path fetch ran with an empty instance set — `list`/`watch`, a `--instance`-pinned fetch, or a mix-mode fallback after FxTwitter failed | Same as above: configure an instance first. Note that mix mode (default) serves `user`/`search`/`get`/`comments`/`following`/`profile`/`quotes`/`trends` without any instance |
