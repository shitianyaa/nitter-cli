...
by the installed binary's `twitter <command> --help`; error messages themselves
...
| `fail(timeout)` / `unreachable` in `instances test`, or transport errors on fetch | 1 | Instance unreachable (network, proxy, wrong URL) | Verify URL reachability, `--proxy`/config proxy; test with `twitter instances test <url>` |
| RSS probes `ok` but `search` empty or `fail(...)` | 0/1 | Search is a separate Nitter capability, disabled or slow on this instance | Run `twitter instances test --full`; use an instance with search enabled |
| `search "#AI"` returns nothing though the tag exists | 0 | Two candidates: instance search disabled, or the query was pre-escaped | Prefer the raw form (`twitter search "#AI"`); the pre-escaped `%23` form double-escapes and must not be used anywhere (`watch tag:` sources included) |
...
| `error: unknown command "..." for "twitter"` | 1 | Typo'd subcommand | Check `twitter --help`; note this exits 1, not 2 |
| `unknown flag: --...` | 2 | Invented or misspelled flag | Run `twitter <command> --help`; never invent flags |
...
2. For fetch problems: `twitter instances test [--full]` and read the cells.
3. For watch problems: `twitter seen list` (default state location only) and
