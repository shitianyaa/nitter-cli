...
//
// Package boundary (frozen): appapi may import the sdk (package nitter) and
...
func newClient(h *httpx.Client, ch *nitter.Chooser, now func() time.Time) (*Client, error) {
	if h == nil {
		var err error
		if h, err = httpx.New(httpx.Options{}); err != nil {
			return nil, nitter.Errorf(nitter.KindLocalState, opNewClient, "build default transport: %w", err)
		}
	}
	if now == nil {
		now = time.Now
