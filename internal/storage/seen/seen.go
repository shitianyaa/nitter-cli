...
// opLoad and opSave are the Op values stamped on seen-store errors. The
// package-qualified form matches every other classified error in this repo
// (media.FetchToFile, appapi.Search); an earlier prose form ("load seen
// state") was the only op that did not.
const (
	opLoad = "seen.Load"
	opSave = "seen.Save"
)
...
func loadFile(path string) (File, error) {
	...
	if err != nil {
		...
		return File{}, nitter.Errorf(nitter.KindLocalState, opLoad,
			"read %s: %w", path, err)
	}
...
func (s *Store) Put(source string, state SourceState) error {
	if source == "" {
		return nitter.Errorf(nitter.KindLocalState, opSave, "empty source key")
	}
...
func (s *Store) persistLocked(sources map[string]SourceState) error {
	...
	if err != nil {
		return nitter.Errorf(nitter.KindLocalState, opSave, "encode: %w", err)
	}
	if err := writeFileAtomic(s.path, data); err != nil {
		return nitter.Errorf(nitter.KindLocalState, opSave, "%w", err)
	}
